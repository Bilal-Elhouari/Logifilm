...
interface SelectPerDayWeekWindowsProps {
  value: string;
  onChange: (value: string) => void;
  dark: boolean;
}

interface SelectBankNameWindowsProps {
  value: string;
  onChange: (value: string) => void;
  dark: boolean;
}
...
export default function NewStarterFormWindows() {
  const { name } = useParams();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const jobId = searchParams.get("job");
  const editId = searchParams.get("edit"); // ✅ Edit mode ID

  const [companyId, setCompanyId] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  /* ---------------- DARK MODE AUTO (Windows) ---------------- */
      ...
      try {
        ...
        if (data) {
          setFormData({
            ...
            holidayWorked: data.holiday_worked || "", // Text type now
            travelDay: data.travel_day || "", // Text
            dailyRate: data.daily_rate ? formatCurrency(data.daily_rate) : "",
            allowance: data.living_allowance || "", // Text
            perDiem: data.per_diem || "", // Text
  ...
  const handleChange = (key: string, val: any) => {
    console.log('handleChange called:', key, val);
  ...
  /* ---------------- IF VALIDATION (digits only, max 15) ---------------- */
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 🔍 DEBUG: Voir le payload avant envoi
  ...
  const handleGeneratePDF = () => {
    ...
    pdf.setFontSize(18);
    pdf.text((name?.toUpperCase() || "COMPANY NAME"), 105, y, { align: "center" }); // Fix title
    ...
    pdf.save(`Starter_${name || "Company"}_${projectTitle || ""}.pdf`);
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-5xl flex justify-between mb-6">
        ...
        <div className="flex gap-4">
          ...
          <motion.button
            ...
            disabled={saving}
            className={`
              flex items-center gap-2 px-6 py-2 rounded-lg text-sm font-medium shadow-md transition
              ${dark
          ...
          <motion.button
            ...
            className={`
              flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-medium shadow-md transition
              ${dark
      ...
      <div
        className={`
          w-full max-w-5xl rounded-3xl border shadow-2xl p-10 transition
          ${dark
            ? "bg-white/5 border-white/15 backdrop-blur-2xl"
        ...
        <div className="grid grid-cols-2 gap-8 mt-10 text-sm">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <StaticSignatureWindows key={lbl} dark={dark} label={lbl} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col relative" ref={dropdownRef}>
      ...
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={`
          w-full px-3 py-2 rounded-lg border text-sm flex justify-between items-center transition
          ${dark
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`
              absolute top-12 w-full rounded-lg overflow-hidden border shadow-xl z-10
    ...
    <div className="flex flex-col relative" ref={dropdownRef}>
      ...
      {showCustom && (
        <input
          ...
          className={`
            mt-2 px-3 py-2 rounded-lg border text-sm
            ${dark
...
/* ---------------- SIGNATURE LINE WINDOWS ---------------- */
function StaticSignatureWindows({ label, dark }: StaticSignatureWindowsProps) {
  return (
    <div className="flex flex-col">
      <label
        className={`font-semibold mb-1 text-xs ${dark ? "text-gray-200" : "text-gray-800"
      ...
      <div className="flex items-center justify-between">
        <div
          className={`flex-1 h-7 rounded-lg border ${dark ? "border-white/25 bg-white/5" : "border-gray-300 bg-gray-100"
            }`}
        ></div>
